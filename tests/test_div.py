...
class DivTest(unittest.TestCase):
    def setUp(self):
        ...
        self.y = div(self.x0, self.x1)
        self.y.backward()

    def test_forward(self):
        result = self.y.data
        expect = numpy.array(2.0 / 3.0)
        self.assertTrue(numpy.allclose(result, expect))

    def test_backward(self):
        result0 = self.x0.grad
        result1 = self.x1.grad
        expect0 = numpy.array(1.0 / 3.0)
        expect1 = numpy.array(-2.0 / 3.0 ** 2)
    ...
    def test_gradient(self):
        ...
        grads_num = numerical_diff(div, self.x0, self.x1)
